...
void Jpeg_ScheduleDecoderTask(JpegContext* ctx) {
    ...
    workBuf->taskData.address = (uintptr_t)PHYSICAL_TO_VIRTUAL(&workBuf->data);
    workBuf->taskData.mode = ctx->mode;
    workBuf->taskData.mbCount = 4;
    workBuf->taskData.qTableYPtr = (uintptr_t)PHYSICAL_TO_VIRTUAL(&workBuf->qTableY);
    workBuf->taskData.qTableUPtr = (uintptr_t)PHYSICAL_TO_VIRTUAL(&workBuf->qTableU);
    workBuf->taskData.qTableVPtr = (uintptr_t)PHYSICAL_TO_VIRTUAL(&workBuf->qTableV);
    ...
    for (i = 0; i < 300; i += 4) {
        if (JpegDecoder_Decode(&decoder, (u16*)workBuff->data, 4, i != 0, &state)) {
            ...
            for (j = 0; j < ARRAY_COUNT(workBuff->data); j++) {
                Jpeg_CopyToZbuffer(workBuff->data[j], (u16*)zbuffer, x, y);
                x++;
